...
│   └── scope-drift.ts        # checkScopeDrift() — detect files modified outside declared scope
│
└── steps/
...
| `code SCRUM-123`          | alias                   | —                 | —      | Chains code-impl → code-test → code-quality → code-security → code-perf (auto-approves each). **Trivial-skip:** Changes ≤10 lines on allowlisted files (`.md`, `.css`, `.json`, `.yaml`, `.toml`) with no security-sensitive paths and passing `tsc --noEmit` skip test/security/perf steps — saves ~$8+ per trivial change. **Scope-drift:** After code-impl, extracts declared files from design.md and compares against `git diff --name-only` — warns on out-of-scope changes |
