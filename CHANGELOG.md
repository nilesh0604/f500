...
- **CI: Lighthouse CI 404 — wrong artifact path and missing SPA flag** — Vite outputs to `apps/vyasa-ui/dist/` (relative to its `cwd`), not `dist/`. Fixed upload/download artifact paths in `pr-checks.yml` (`build`, `lighthouse`, `bundle-size` jobs) to use `apps/vyasa-ui/dist/`. Fixed `lighthouserc.js` serve path to `apps/vyasa-ui/dist` and added `-s` flag to `npx serve` so client-side React Router routes (`/auth/login`, `/auth/register`) are correctly served from `index.html` instead of returning 404.
